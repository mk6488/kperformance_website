...
const ADMIN_COOLDOWN_MS = 30_000;
...
type ClinicianSummaryJson = {
  presentingSnapshot: string;
  workingHypothesis: string;
  differentials: { name: string; rationale: string }[];
  plan: {
    handsOn: string[];
    movementLoad: string[];
    education: string[];
...
function formatClinicianSummaryMarkdown(json: ClinicianSummaryJson) {
  const lines: string[] = [];
  ...
  if (Array.isArray(json.differentials) && json.differentials.length > 0) {
    json.differentials.forEach((d) => {
      ...
      if (name && rationale) lines.push(`- **${name}**: ${rationale}`);
...
function isClinicianSummaryJson(v: any): v is ClinicianSummaryJson {
  if (!v || typeof v !== 'object') return false;
  if (typeof v.presentingSnapshot !== 'string') return false;
  if (typeof v.workingHypothesis !== 'string') return false;
  if (!Array.isArray(v.differentials)) return false;
  if (!v.plan || typeof v.plan !== 'object') return false;
  if (!Array.isArray(v.referralTriggers)) return false;
  return true;
}
...
export const generateIntakeAIReport = onCall({ region: 'europe-west2' }, async (request) => {
  ...
  const readDayCounts = (data: any) => {
    const countSuccess =
      typeof data?.countSuccess === 'number'
        ? data.countSuccess
        : typeof data?.count === 'number'
          ? data.count
          : 0;
    const countPending =
      typeof data?.countPending === 'number'
        ? data.countPending
    ...
    const lastCallAt =
      data?.lastCallAt?.toDate && typeof data.lastCallAt.toDate === 'function'
  ...
  const reserveOrThrow = async () => {
    await db.runTransaction(async (txn) => {
      ...
      const adminData = adminSnap.exists ? (adminSnap.data() as any) : {};
      const intakeUsageData = intakeSnap.exists ? (intakeSnap.data() as any) : {};
      ...
      if (adminCounts.lastCallAt && nowMs - adminCounts.lastCallAt.getTime() < ADMIN_COOLDOWN_MS) {
        throw new HttpsError('failed-precondition', 'Please wait before generating another report.');
      }
      ...
      if (intakeCounts.countSuccess + intakeCounts.countPending >= INTAKE_DAILY_LIMIT) {
        throw new HttpsError('resource-exhausted', 'Daily AI generation limit reached for this intake. Try again tomorrow.');
      }
  ...
  const finaliseReservation = async (outcome: 'success' | 'failure', costUsd?: number | null) => {
    await db.runTransaction(async (txn) => {
      const [adminSnap, intakeSnap] = await Promise.all([txn.get(adminDayRef), txn.get(intakeDayRef)]);
      const adminData = adminSnap.exists ? (adminSnap.data() as any) : {};
      const intakeUsageData = intakeSnap.exists ? (intakeSnap.data() as any) : {};
...
`.trim();
  ...
  const doFetch = async (attempt = 0): Promise<any> => {
    ...
    if (code === 'insufficient_quota') {
      throw new HttpsError('failed-precondition', 'OpenAI quota exceeded.');
    }
  ...
  try {
    ...
    let contentText: string = rawText;
    let contentJson: ClinicianSummaryJson | null = null;
    if (reportType === 'clinician_summary') {
      let parsed: any = null;
      try {
        ...
        if (first !== -1 && last !== -1 && last > first) {
          ...
          throw new HttpsError('internal', 'AI response was not valid JSON for clinician summary.');
        }
      }
      if (!isClinicianSummaryJson(parsed)) {
        throw new HttpsError('internal', 'AI response JSON did not match expected clinician summary schema.');
      }
      contentJson = parsed;
