...
export default function StepRetestPower({ assessment, update }: Props) {
  ...
  return (
    <div className="space-y-4">
      <p className="text-xs text-brand-slate bg-brand-offWhite border border-[#e9eef5] rounded-lg px-3 py-2.5">
        Open My Jump Lab → perform test → return here → enter result. Same camera position as initial session.
      </p>
      ...
      <p className="text-xs font-bold tracking-widest uppercase text-brand-blue">Sprint — identical distance and surface to initial (2 trials, best)</p>
      ...
      <div className="space-y-1.5">
        ...
        <textarea className={textareaCls} value={pw.sprintNotes ?? ''} onChange={e => update(a => ({ ...a, power: { ...a.power, sprintNotes: e.target.value || null } }))} placeholder="e.g. Conditions different from initial — note any confounds" />
      </div>
